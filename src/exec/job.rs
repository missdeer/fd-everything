...
use crate::scan::sink::WorkerResult;
...
pub(crate) fn job(
    ...
    // PLAN.md §2.X: placeholder substitution uses the projected path.
    // Build the projector once per worker — it's cheap (just borrows
    // Config) and avoids re-sorting search roots on every hit.
    ...
    for result in results {
        ...
        let projected = projector.project_for_output(dir_entry.path());
...
pub(crate) fn batch(
    ...
    // Same rationale as `job`: each path is projected once via the
    // shared projector before being handed to argv assembly.
    let projector = config.path_projector();
    let paths = results
        .into_iter()
        .filter_map(|worker_result| match worker_result {
            WorkerResult::Entry(dir_entry) => {
                Some(projector.project_for_output(dir_entry.path()).into_owned())
            }
            WorkerResult::Error(err) => {
                if config.show_filesystem_errors {
                    print_error(err.to_string());
